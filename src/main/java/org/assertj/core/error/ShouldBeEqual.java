...
public class ShouldBeEqual implements AssertionErrorFactory {
   ...
   */
  public static AssertionErrorFactory shouldBeEqual(Object actual, Object expected, Presentation presentation) {
    return new ShouldBeEqual(actual, expected, StandardComparisonStrategy.instance(), presentation);
  }
  ...
  public static AssertionErrorFactory shouldBeEqual(Object actual, Object expected,
                                                     ComparisonStrategy comparisonStrategy, Presentation presentation) {
    return new ShouldBeEqual(actual, expected, comparisonStrategy, presentation);
  }

  @VisibleForTesting
  ShouldBeEqual(Object actual, Object expected, ComparisonStrategy comparisonStrategy, Presentation presentation) {
    this.actual = actual;
   ...
   */
  @Override
  public AssertionError newAssertionError(Description description, Presentation presentation) {
    if (actualAndExpectedHaveSameStringRepresentation()) {
      ...
      return Failures.instance().failure(defaultDetailedErrorMessage(description, presentation));
    }
    ...
    return Failures.instance().failure(defaultErrorMessage(description, presentation));
  }

  /**
   * Tells {@link AssertionErrorFactory#newAssertionError(org.assertj.core.description.Description, org.assertj.core.presentation.Presentation)} if it should try a build a {@link org.junit.ComparisonFailure}.<br>
  ...
  private boolean actualAndExpectedHaveSameStringRepresentation() {
    return areEqual(presentation.toStringOf(actual), presentation.toStringOf(expected));
  }
   ...
   * @param presentation the {@link Presentation} used to build String representation of object
   * @return the error message from description using {@link #expected} and {@link #actual} basic representation.
   */
  private String defaultErrorMessage(Description description, Presentation presentation) {
    if (comparisonStrategy instanceof ComparatorBasedComparisonStrategy)
      return messageFormatter
               .format(description, presentation, EXPECTED_BUT_WAS_MESSAGE_USING_COMPARATOR, actual, expected, comparisonStrategy);
    return messageFormatter.format(description, presentation, EXPECTED_BUT_WAS_MESSAGE, expected, actual);
  }
   ...
   */
  private String defaultDetailedErrorMessage(Description description, Presentation presentation) {
    if (comparisonStrategy instanceof ComparatorBasedComparisonStrategy)
      return messageFormatter.format(description,presentation, EXPECTED_BUT_WAS_MESSAGE_USING_COMPARATOR, detailedActual(),
                                     detailedExpected(), comparisonStrategy);
    return messageFormatter.format(description, presentation, EXPECTED_BUT_WAS_MESSAGE, detailedExpected(), detailedActual());
  }
  ...
  private Object[] msgArgs(String description) {
    return array(description, presentation.toStringOf(expected), presentation.toStringOf(actual));
  }

  private String detailedToStringOf(Object obj) {
    return presentation.toStringOf(obj) + " (" + obj.getClass().getSimpleName() + "@" + toHexString(obj.hashCode()) + ")";
  }
