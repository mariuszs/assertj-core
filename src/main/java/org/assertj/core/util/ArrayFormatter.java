...
final class ArrayFormatter {
  ...
  String format(Presentation p, Object o) {
    if (!isArray(o)) {
      return null;
    }
  ...
  private String formatObjectArray(Presentation p, Object o) {
  ...
  private void deepToString(Presentation p, Object[] array, StringBuilder buffer, Set<Object[]> alreadyFormatted) {
    if (array == null) {
      buffer.append(NULL);
      return;
    }
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (!isArray(element)) {
        buffer.append(element == null ? NULL : p.arrayElementToStringOf(element));
        continue;
      }
  ...
  private String formatPrimitiveArray(Presentation p, Object o) {
    if (!isArray(o)) {
      return null;
    }
    ...
    buffer.append('[');
    buffer.append(p.arrayElementToStringOf(Array.get(o, 0)));
    for (int i = 1; i < size; i++) {
      Object element = Array.get(o, i);
      buffer.append(p.elementSeparator(element));
      buffer.append(p.arrayElementToStringOf(element));
    }
    buffer.append("]");
    return buffer.toString();
  }
